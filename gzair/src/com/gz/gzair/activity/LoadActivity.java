...
import com.gz.gzair.util.UpdateManager;
...
public class LoadActivity   extends BaseActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if(repMessage!=null&&!("notupd".equals(repMessage))&&!("timeout".equals(repMessage))){//需要升级的，程序就停在首页等待升级，升级过以后才能进行下一步。
                //这里来检测版本是否需要更新  
        ...
        if("".equals(mac)){//如果手机里面没有mac提示用户用wifi的方式联网，来获取mac地址
            ...
            if(bool){//是wifi联网
                   ...
                   tt.start();
                    ...
                    Toast.makeText(this, this.getResources().getString(R.string.notwifi),Toast.LENGTH_LONG).show();
            }
        }else{
            Thread tt=new Thread(new TimeThread());
            tt.start();
    ...
    /***
    *If current return value equal to NET_TYPE_FULLCONE_NAT, NET_TYPE_REST_NAT and NET_TYPE_PORTREST_NAT, it can support UDP Hole Punching
    *
	*NET_TYPE_OPENED = 0,    
	*NET_TYPE_FULLCONE_NAT = 1,    // Full Cone NAT
	*NET_TYPE_REST_NAT = 2,    // Restricted Cone NAT (restrict IP)
	*NET_TYPE_PORTREST_NAT = 3,     // Port Restricted Cone NAT (restrict IP & Port)
